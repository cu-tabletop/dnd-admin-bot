...
class TestRatingFunctionality:
    ...
    async def test_rating_input_success(self):
        ...
        with patch("services.api_client.get_api_client") as mock_api:
            ...
            mock_client.update_character_rating.return_value = Character(
                id=1, name="Test", level=5, rating=95, company_id=1
            )
    ...
    async def test_quick_rating_change(self):
        ...
        with patch("services.api_client.create_api_client") as mock_api:
            ...
            mock_client.get_character.return_value = Character(
                id=1, name="Test", level=5, rating=50, company_id=1
            )
            mock_client.update_character_rating.return_value = Character(
                id=1, name="Test", level=5, rating=55, company_id=1
            )
            mock_api.return_value = mock_client
...
class TestStubAPIRating:
    ...
    async def test_stub_rating_update(self):
        ...
        # Обновляем рейтинг
        updated_character = await stub_client.update_character_rating(
            1, initial_rating + 10
        )
